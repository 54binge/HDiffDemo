...
int generatePatch(int argc, char **argv) {
    clock_t time0 = clock();
    if (argc != 4) {
    ...
    readFile(oldData, oldFileName);
    std::vector<TByte> newData;
    readFile(newData, newFileName);
    ...
    TByte *newData_begin = 0;
    if (!newData.empty()) newData_begin = &newData[0];
    const TByte *oldData_begin = 0;
    if (!oldData.empty()) oldData_begin = &oldData[0];
    clock_t time1 = clock();
    create_diff(newData_begin, newData_begin + newDataSize, oldData_begin,
                oldData_begin + oldDataSize, diffData);
    clock_t time2 = clock();
    if (!check_diff(newData_begin, newData_begin + newDataSize, oldData_begin,
                    oldData_begin + oldDataSize, &diffData[0] + kNewDataSize,
                    &diffData[0] + diffData.size())) {
...
JNIEXPORT jint JNICALL Java_example_binge_com_hdiffdemo_HDiffPatch_hdiff(JNIEnv *env, jobject thiz, jstring oldFilePath,
                                                  jstring newFilePath,
